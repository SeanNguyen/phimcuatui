...
var (
    ...
    // Command-line flags
    seed        = flag.String("seed", "http://phimmoi.net", "seed URL")
...
func getVideoLink(ctx *fetchbot.Context, doc *goquery.Document) {
    doc.Find("video[src]").Each(func(i int, s *goquery.Selection) {
        val, _ := s.Attr("src")
        // Resolve address
        videoSrc, err := ctx.Cmd.URL().Parse(val)
        if err != nil {
            fmt.Printf("error: resolve video source %s - %s\n", val, err)
            return
        }
        fmt.Print(videoSrc)
    })
